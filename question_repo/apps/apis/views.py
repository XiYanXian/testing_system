...
from apps.repo.models import Questions, Answers, AnswersCollection, UserLog
...
from apps.repo.models import QuestionsCollection, Answers
...
class QuestionsView(View):
    def get(self, request):
        ...
        if search:
            if search.isdigit():
                questions_list = questions_list.filter(Q(id=search) | Q(
                    content__icontains=search) | Q(title__icontains=search))
            else:
        ...
        # 筛选状态 => 我的答题表
        questions_list = questions_list.values(
            'id', 'title', 'grade', 'answer')
...
class QuestionCollectionView(LoginRequiredMixin, View):
    def get(self, request, id):
        ...
        result = QuestionsCollection.objects.get_or_create(
        ...
        if not result[1]:
            ...
            if question_collection.status:
                ...
                UserLog.objects.create(user=request.user, operate=2, question=question)
            else:
                question_collection.status = True
                UserLog.objects.create(user=request.user, operate=1, question=question)
    ...
    def get(self, request, id):
        ...
        if other_answer:
            for answer in other_answer:
                if AnswersCollection.objects.filter(
                        answer=answer, user=request.user, status=True):
                ...
                answer.collect_nums = answer.answers_collection_set.filter(
                    status=True).count()
...
class AnswerCollectionView(LoginRequiredMixin, View):
    def get(self, request, id):
        ...
        result = AnswersCollection.objects.get_or_create(
        ...
        if not result[1]:
            ...
            if answer_collection.status:
                ...
                UserLog.objects.create(user=request.user, operate=2, answer=answer)
            else:
                answer_collection.status = True
                UserLog.objects.create(user=request.user, operate=1, answer=answer)
        answer_collection.save()
        msg = model_to_dict(answer_collection)
        msg["collections"] = answer.answers_collection_set.filter(
            status=True).count()
        ret_info = {"code": 200, "msg": msg}
        return JsonResponse(ret_info)
