...
class Questions(models.Model):
    ...
    grade = models.IntegerField('题目难度', choices=DIF_CHOICE, validators=[
                                valid_difficulty], null=True, blank=True)
    category = models.ForeignKey(Category, verbose_name='分类', null=True)
    title = models.CharField('题目标题', unique=True, max_length=256)
    # content = models.TextField('题目详情', null=True, blank=True)
    ...
    contributor = models.ForeignKey(
        User, verbose_name="贡献者", null=True, blank=True)
    ...
    class Meta:
        ...
        verbose_name_plural = verbose_name
        permissions = (
            ('can_change_question', '可以修改题目'),
            ('can_add_question', '可以添加题目'),
...
class QuestionsCollection(models.Model):
    ...
    question = models.ForeignKey(
        Questions,
        verbose_name="问题",
        related_name='questions_collection_set')
...
class AnswersManager(models.Manager):
    ...
    def hot_user(self):
        ...
        user_rank = self.filter(last_modify__gte=today_30).values(
            'user__username').annotate(Count('id')).order_by("-id__count")[:5]
        return user_rank
...
class AnswersCollection(models.Model):
    ...
    answer = models.ForeignKey(
        Answers,
        verbose_name="答题记录",
        related_name='answers_collection_set')
    user = models.ForeignKey(
        User,
        verbose_name="收藏者",
        related_name='answers_collection_set')
...
class UserLog(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.question or self.answer and not (self.question and self.answer):
            super().save()
        else:
